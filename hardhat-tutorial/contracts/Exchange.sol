...
contract Exchange is ERC20 {
    ...
    // exchange is inheriting ERC20, because our exchange would keep track of crypto dev LP token
    constructor(address _CryptoDevtoken) ERC20("CrytoDev LP Token", "CDLP") {
        require(
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        if (cryptoDevTokenReserve == 0) {
            ...
            /**
                ...
                is adding Eth to the contract, so whatever Eth contract has is equal to the one supplied by
                the user in the current `addLiquidity` call
            ...
            /**
                if the reserve is not empty, intake any user supplied value for `ether` and determine according 
                to the ratio of how many `Crypto Dev` tokens need to be supplied to prevent any large price impacts 
                because of additional liquidity
             */

            // ethReserve should be the current ethBalance subtracted by the value of ether
            // sent by the user in current `addLiquidity` call
            uint ethReserve = ethBalance - msg.value;

            /**
                ration should always be maintained so that there are no major price impacts when adding liquidity
                ratio here is -> (cryptoDevTokenAmount user can add/cryptoDevTokenReserve in contract) = (eth sent by user/eth reserve in contract)
                so doing some maths, (cryptoDevTokenAmount user can add) = (Eth sent by user * cryptoDevTokenReserve in contract/Eth Reserve)
              */
            ...
            //transfer only (cryptoDevTokenAmount user can add) amount of CD token from user acct to contract
            ...
            /**
                the amount of LP tokens that would be sent to user should be proportional to liquidity of
                ether added by user
                ratio here to maintain is ->
                (LP token to be sent to user(liquidity)/ totalSupply of LP tokens in contract) = (eth sent by the user/eth reserve in the contract)
    ...
    function removeLiquidity(uint _amount) public returns (uint, uint) {
        ...
        // transfer ethAmount of eth from contract to user's wallet
        payable(msg.sender).transfer(ethAmount);
        // transfer cryptoDevTokenAddress of crypto dev token from contract to user's wallet
    ...
    function ethToCryptoDevToken(uint _minTokens) public payable {
        ...
        // transfer the crypto dev tokens to user
