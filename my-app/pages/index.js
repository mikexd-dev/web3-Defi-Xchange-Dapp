...
export default function Home() {
  ...
  const _getAmountOfTokensReceivedFromSwap = async (_swapAmount) => {
    try {
      // convert the amount entered by the user to a BigNumber using the `parseEther` lib from ether.js
      const _swapAmountWEI = utils.parseEther(_swapAmount.toString());
      // check if the user entered zero using eq method from BigNumber class in ether.js
  ...
  const _addLiquidity = async () => {
    try {
      console.log(addEther, addCDTokens);
